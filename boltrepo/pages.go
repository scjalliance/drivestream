...
func (seq Pages) Next() (n page.SeqNum, err error) {
	err = seq.db.View(func(tx *bolt.Tx) error {
		...
		if len(k) != 8 {
			...
			return BadPageKey{Drive: seq.drive, Collection: seq.collection, BadKey: key}
		}
...
func (seq Pages) Read(start page.SeqNum, p []page.Data) (n int, err error) {
	err = seq.db.View(func(tx *bolt.Tx) error {
		...
		pages := col.Bucket([]byte(PageBucket))
		if pages == nil {
			return collection.PageNotFound{Drive: seq.drive, Collection: seq.collection, Page: start}
		}
		cursor := pages.Cursor()
		pos := start
		key := makePageKey(pos)
		...
		for n < len(p) {
			if v == nil {
				return collection.PageDataInvalid{Drive: seq.drive, Collection: seq.collection, Page: pos} // All pages must be non-nil
			}
...
func (seq Pages) Ref(seqNum page.SeqNum) page.Reference {
	return Page{
		db:         seq.db,
		drive:      seq.drive,
...
func (seq Pages) Clear() error {
	return seq.db.Update(func(tx *bolt.Tx) error {
		col := collectionBucket(tx, seq.drive, seq.collection)
		if col == nil {
			return collection.NotFound{Drive: seq.drive, Collection: seq.collection}
		}
