...
/*
func listPermissions(ctx context.Context, s *drive.Service, id string) (perms []Permission, err error) {
	var token string
	...
	for {
		...
		for _, perm := range list.Permissions {
			...
			if perm.ExpirationTime != "" {
				expiration, err = time.Parse(time.RFC3339, perm.ExpirationTime)
				if err != nil {
					return nil, fmt.Errorf("invalid expiration time for permission %s: %v", perm.Id, err)
				}
			}
...
/*
func showPermissionDetail(ctx context.Context, s *drive.Service, driveID, permissionID string) {
	call := s.Permissions.Get(driveID, permissionID)
	call.Context(ctx)
	call.SupportsTeamDrives(true)
	call.UseDomainAdminAccess(true)
	call.Do()
	...
	perm, err := call.Do()
	if err != nil {
		fmt.Printf("ERROR: Unable to query permission details: %v\n", err)
		return
	}
	...
	expiration := ""
	if perm.ExpirationTime != "" {
		expiration = fmt.Sprintf(", expires: %s", perm.ExpirationTime)
	}

	fmt.Printf("  PERMISSION %s: %s (%s%s | name: \"%s\", email: \"%s\", domain: \"%s\"%s)\n", perm.Id, perm.Role, deleted, strings.ToUpper(perm.Type), perm.DisplayName, perm.EmailAddress, perm.Domain, expiration)
}
*/
